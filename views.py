            ...
            if user_message == "main menu":
                ...
                if(user_selected_option == "FAR Information".lower() ):
                    ...
                    if user_message == "yes":
                        ...
                        arr=user_message.values()
                        all_arr_empty=all(not s for s in arr)
                        if(all_arr_empty):
                            bot_response={"Subject":"","Source":"","Destination":"","Zone":"","Port":"","expiryoptions":["before","after"],"expiry":"date","createdoptions":["before","after"],"created":"date"}
                        #bot_response="FORM"
                        else:
                            bot_response,option_button = get_far_db(request,user_dict)
                    else:
                        print("else far multiple list",user_message)
